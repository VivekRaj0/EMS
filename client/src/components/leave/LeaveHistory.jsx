                      ...
                      <td>
                        {leave.status === "PENDING" && (
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => handleStatusUpdate(leave._id || leave.id, "REJECTED")} disabled={!!processing} className="p-1.5 rounded-md bg-emerald-50 text-rose-600 hover:bg-rose-100 transition-colors">
                              {processing === (leave.id || leave._id) ? (
                                <Loader2 className="w-4 h-4 animate-spin" />
                              ) : (
                                <X className="w-4 h-4" />
